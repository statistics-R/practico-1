...
-   [01-class.zip](https://github.com/learn-R/02-class/raw/main/02-class.zip)
-   Apéndice B - Woolridge

## 1. Variables aleatorias y sus distribuciones de probabilidad

Las Variables Aleatorias (VA) son **variables estadísticas** en la que sus valores se obtienen a partir del resultado de algún tipo de experimento. En otras palabras, son variables cuyos valores son **resultados numéricos** de un fenómeno aleatorio. Se llama **fenómeno aleatorio** cuando los resultados individuales son inciertos y, sin embargo, existe una distribución regular de los resultados después de un gran número de repeticiones. En este sentido, **la probabilidad** de cualquier resultado de un fenómeno aleatorio es la proporción de veces que el resultado se da después de una larga serie de repeticiones.

Las variables aleatorias siempre están definidas para tomar valores numéricos, aun cuando describan eventos cualitativos, por ejemplo cuando lanzamos una moneda los únicos resultados posibles son cara o cruz, donde \$\$X=1\$\$ si se obtiene cara y $$X=0$\$ si se obtiene cruz. Estas **se dividen en discretas o continuas**.

Las **VA discretas (VAD)** se describen estadísticamente por su **distribución de probabilidad**. La distribución de probabilidad de una variable aleatoria $$X$\$ nos dice qué valores puede tomar $$X$\$ y cómo asignar probabilidades a estos valores, es decir, nos entrega un listado de todos los valores posibles que puede tomar una VA y la probabilidad de ocurrencia de cada valor. La suma es igual a 1.
...
***Tabla 2: Distribución de Probabilidad Acumulada***

+--------------------+--------------------------+
| Hijos              | Prob. Acumulada          |
+:==================:+:========================:+
| 0                  | 27.60                    |
+--------------------+--------------------------+
| 1                  | 57.37                    |
...
***Gráfico 4: Distribución de Probabilidad Acumulada***

![](images/grafico04-01.png){width="436"}

## 2. Distribuciones conjuntas, condicionales, marginales e independencia

### 2.1 Distribución Conjunta

En muchos casos, resulta interesante estudiar conjuntamente el comportamiento de las variables. En dichos caso, se construye lo que se denomina **distribución conjunta**. Su objetivo consiste en determinar cómo la variación de una variable se relaciona con la de otra. En el caso de que $$X$\$ y $$Y$\$ sean variables aleatorias discretas, entonces $$(X,Y)$\$ tienen una distribución conjunta, descrita completamente por la **función de densidad de probabilidad conjunta** de $$(X,Y)$\$.
